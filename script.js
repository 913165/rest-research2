...
import setupEditors from "./response"
//import mysql from "mysql2"

const form = document.querySelector("[data-form]")
const queryParamsContainer = document.querySelector("[data-query-params]")
const requestHeadersContainer = document.querySelector("[data-request-headers]")
const keyValueTemplate = document.querySelector("[key-value-template]")
...
function saveRequest() {
  ...
  const requestHeaders = keyValuePairsToObjects(requestHeadersContainer);
  const requestData = JSON.parse(requestEditor.state.doc.toString() || null);

  const savedRequestsList = document.getElementById("saved-requests");
  const listItem = document.createElement("li");
  const link = document.createElement("a");
  link.href = "/";

  link.textContent = `${method}-${url}`;
